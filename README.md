...
## Прелюдия: на какие вопросы по тикеру отвечает контур новостей

Даже при сильных офлайн-метриках новостной слой — это **не замена** брокерскому решению «купить/продать по цене». Он заточен под другое:

**На что можно опираться (новости ± последующий fusion с техникой):**

- **Содержание потока:** какие темы и риски доминируют по тикеру за окно времени; согласованность источников.  
- **Структурированный импульс:** направление и сила сигнала в вашей шкале (bias / score), опционально по горизонтам (1d / 3d / 5d и т.д.) — как **ранжирующий** индикатор, не гарантия доходности.  
- **Уверенность:** после калибровки на истории — более честная интерпретация «насколько это типично сбывалось» в вашей постановке.  
- **Сводка с техникой:** согласованы ли новости с техническим сигналом или конфликтуют (режим осторожности vs подтверждение).

**Чего ждать не стоит без явного подключения цены, исполнения и риск-правил:**

- точные цели/стопы в пунктах «из одних новостей»;  
- гарантированный результат;  
- ответы, требующие инсайда или непубличных данных.

Практическая цель: **перевод информационного потока в измеримый импульс** и **совместное решение** с теханализом, а не автономный торговый оракул.

## Сестринские проекты

| Проект | Роль |
|--------|------|
| **[lse](https://github.com/kobyzev-yuri/lse)** | Торговый контур, песочница GAME_5M, PostgreSQL (`knowledge_base`, котировки), бот и кроны; миграции расширения KB и свечей: `db/knowledge_pg/`. |
| **[tradenews](https://github.com/kobyzev-yuri/tradenews)** | Офлайн-бенчмарк LLM по новостям vs forward log-return, ProxyAPI/Ollama, отчёты и план импульса: `docs/news_impulse_plan.md`. |

Репозиторий **nyse** задаёт пайплайн нормализации новостей и LLM-сигнала; **lse** подключает хранение, игру и прод-интеграции; **tradenews** — сравнение моделей и методология метрик на тех же идеях, что L5-промпт в nyse.
